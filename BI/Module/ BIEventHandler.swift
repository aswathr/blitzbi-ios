...
public protocol PBiEventSendHandler {
...
class BiEventSendHandler:NSObject, PBiEventSendHandler {
    ...
    private var eventRepository:IBIEventRepository
    
    public init(batchSize:Int, baseUrl: String?, eventsUrl: String?, biEventsDirectUrl: String?,eventRepository:IBIEventRepository ) {
                    ...
                    if JSONSerialization.isValidJSONObject(batch) {
                        ...
                        eventRepository.processJsonRequest(url:url ?? "", data: jsonData, completionHandler: { [self] response, err in
                            ...
                            if err != nil || !success {
                                ...
                                BiEventSendHandler.sendCrashlyticsError(withCode: BI_SERVER_ERROR_CODE, withDescription: BI_SERVER_ERROR, withUserInfo: (err as NSError?)?.userInfo)
                            }
                            semaphore.signal()
                        })
    ...
    private func eventsFilePath() -> String? {
        return FileHelper.fullFilePathInDocumentsDirectory(forFileName: EVENTS_FILE_PATH)
    }
    ...
    private func removeDefectiveEvents() {
        ...
        BiEventSendHandler.sendCrashlyticsError(
        ...
        do {
            if let object = object {
                if try !NSKeyedArchiver.archiveRootObject(object, toFile: filePath ?? "") {
                    BiEventSendHandler.sendCrashlyticsError(withCode: BI_ARCHIVE_DATA_FAILURE_CODE, withDescription: BI_ARCHIVE_DATA_FAILURE, withUserInfo: nil)
                    return false
                }
            }
    ...
    private func addSkipBackupAttributeToItem(atPath filePathString: String?) -> Bool {
        ...
        if fileExists {
            ...
            do {
                ...
                BiEventSendHandler.sendCrashlyticsError(withCode: BI_EXCLUDING_FROM_BACKUP_ERROR_CODE, withDescription: BI_EXCLUDING_FROM_BACKUP_ERROR, withUserInfo: userInfo)
            }
            return success
        }
        BiEventSendHandler.sendCrashlyticsError(withCode: BI_FILE_NOT_EXISTS_ERROR_CODE, withDescription: BI_FILE_NOT_EXISTS_ERROR, withUserInfo: [
    ...
    private func unarchiveEvents() {
        pendingEvents = BiEventSendHandler.unarchiveOrDefault(fromFile: eventsFilePath(), as: NSMutableArray.self) as? [[String:Any]]
    ...
    class func unarchive(fromFile filePath: String?, as `class`: AnyClass) -> Any? {
        ...
        do {
            ...
            do {
                ...
                BiEventSendHandler.sendCrashlyticsError(withCode: BI_FILE_REMOVAL_FAILURE_CODE, withDescription: BI_FILE_REMOVAL_FAILURE, withUserInfo: userInfo)
            }
        }
    ...
    private func getJSONData(forBatch batch: [[AnyHashable:Any]]?) -> Data? {
        ...
        if jsonData == nil || error != nil {
            ...
            BiEventSendHandler.sendCrashlyticsError(withCode: BI_JSON_DATA_ERROR_CODE, withDescription: BI_JSON_DATA_ERROR, withUserInfo: (error as NSError?)?.userInfo)
        }
        return jsonData
    }
