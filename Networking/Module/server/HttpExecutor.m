...
+ (AFURLSessionManager *)getSessionManager {
    ...
    @synchronized(self) {
        if (manager == nil) {
            ...
            [HttpExecutor applyPinningToSessionManager:manager];
...
+ (AFURLSessionManager *)getBISessionManager {
    ...
    @synchronized(self) {
        if (biManager == nil) {
            ...
            [HttpExecutor applyPinningToSessionManager:biManager];
...
+ (AFURLSessionManager *)getLogSessionManager {
    ...
    @synchronized(self) {
        if (logManager == nil) {
            ...
            [HttpExecutor applyPinningToSessionManager:logManager];
...
+ (void)executeParallelRequest:(RequestBuilder *)requestBuilder listener:(id<HttpResponseListener> )listener {

    AFURLSessionManager *manager = [self getSessionManagerWithNoConnectionLimit];
    [HttpExecutor fire:manager withRequest:requestBuilder listener:listener];
}
...
+ (void)executeBIRequest:(RequestBuilder *)requestBuilder listener:(id<HttpResponseListener> )listener {
    ...
    [HttpExecutor fire:biManager withRequest:requestBuilder listener:listener];
}

+ (void)executeLogRequest:(RequestBuilder *)requestBuilder listener:(id<HttpResponseListener> )listener {
    AFURLSessionManager *logManager = [self getLogSessionManager];
    [HttpExecutor fire:logManager withRequest:requestBuilder listener:listener];
}

+ (void)fire:(AFURLSessionManager *)manager withRequest:(RequestBuilder *)requestBuilder listener:(id<HttpResponseListener> )listener {
...
+ (NSObject *)getParsedData:(NSURLResponse *)response withRequest:(RequestBuilder *)requestBuilder withResponseObject:(id)responseObject andError:(NSError *)error {
    ...
    if (error == nil
        ...
        NSData *responseData = [KiwiParser parse:encryptedResponseObjectStr withDetailsOne:[requestBuilder detailsOne] andDetailsTwo:[requestBuilder detailsTwo]];
