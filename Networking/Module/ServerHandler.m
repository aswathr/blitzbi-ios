...
KWConcurrentDictionary *requestRetry;
...
+ (instancetype)serverHandler {
    static ServerHandler *sharedInstace = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstace = [[ServerHandler alloc] init];
    });
...
- (instancetype)init {
    self = [super init];
    if (self) {
        requestRetry = [[KWConcurrentDictionary alloc] init];
...
- (void)onHttpResponse:(id)response forRequestBuilder:(RequestBuilder *)request error:(NSError *)err withAlertVisibility:(BOOL)visiblityFlag {
        ...
        if ([self arrayContains:RECOVERABLE_ERROR_CODES num:errorCode]) {
            ...
            if (retry < MAX_RETRY) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, DELAY_PER_RETRY), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0ul), ^{
                    ...
                    [HttpExecutor executeRequest:request listener:self];
...
/*show connection error dialog*/
- (BOOL)callShowAlert:(NSError *)err forRequest:(RequestBuilder *)request forResponseCode:(NSString *) responseCode withResponse:(NSDictionary*) response withAlertVisibility:(BOOL)showAlert {
//    NSNumber *errorCode = [NSNumber numberWithInteger:[err code]];
...
- (void)retryPendingCalls {
    @synchronized(self) {
        for (RequestBuilder *req in pendingRequests) {
            [req setPendingRequest:YES];
            [HttpExecutor executeRequest:req listener:self];
        }
...
- (void)sendFailueOrRetryEvent:(RequestBuilder *)requestBuilder error:(NSError *)error isRetry:(BOOL)isRetry {
