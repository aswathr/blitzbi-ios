...
+ (BOOL)fileExistsInBundle:(NSString *)fileName {
    return [FileHelper fileExistsAtPath:[FileHelper fullFilePathInBundleForFileName:fileName]];
}

+ (BOOL)fileExistsInCacheDirectory:(NSString *)fileName {
    return [FileHelper fileExistsAtPath:[FileHelper fullFilePathInCacheDirectoryForFileName:fileName]];
}

+ (BOOL)fileExistsInDocumentsDirectory:(NSString *)fileName {
    return [FileHelper fileExistsAtPath:[FileHelper fullFilePathInDocumentsDirectoryForFileName:fileName]];
}
...
+ (void)deleteCacheDirectoryNamed:(NSString *)dirName {
    [[NSFileManager defaultManager] removeItemAtPath:[[FileHelper applicationCacheDirectoryPath] stringByAppendingString:[@"/" stringByAppendingString:dirName]] error:nil];
}

+ (void)createCacheDirectoryNamed:(NSString *)dirName {
    [[NSFileManager defaultManager] createDirectoryAtPath:[[FileHelper applicationCacheDirectoryPath] stringByAppendingString:[@"/" stringByAppendingString:dirName]] withIntermediateDirectories:NO attributes:nil error:nil];
}
...
+ (BOOL)deleteLocalCacheFileWithName:(NSString *)filename {
    NSURL *fullPath = [FileHelper fullFileUrlInCacheDirectoryForFileName:filename];
...
+ (void)clearAppDirectoriesOnFreshInstall {
    NSString *documentsDirectoryPath = [FileHelper publicDocumentsDirectoryPath];
    NSString *libraryDirectoryPath = [FileHelper publicLibraryDirectoryPath];
    NSString *cacheDirectoryPath = [FileHelper applicationCacheDirectoryPath];

    [FileHelper deleteFilesInDirectory:documentsDirectoryPath];
    [FileHelper deleteFilesInDirectory:libraryDirectoryPath];
    [FileHelper deleteFilesInDirectory:cacheDirectoryPath];
}
