...
+ (NSString *)getFormattedDetails {
    return [NSString stringWithFormat:@"App Version:%@ \nDevice Name:%@ \nCarrier Name:%@ \nConnection:%@ \nIDFA:%@ \nIFB:%@", [DeviceUtils getAppVersion], [DeviceUtils getDeviceModel], [DeviceUtils getCarrierName], [DeviceUtils getConnDetails], [DeviceUtils getIDFA], [DeviceUtils getIFV]];
}
...
+ (NSString *)getKiwiDeviceId {
    NSNumber *kiwiDeviceId = [DeviceUtils getKiwiDeviceIdAsNumber];
    if (kiwiDeviceId) {
...
+ (NSNumber *)getKiwiDeviceIdAsNumber {
    ...
    if(KIWI_DEVICE_ID_VAL == nil) {
        NSNumber *kiwiDeviceId = [[UserDefaultsUtil sharedInstance] objectForKey:KIWI_DEVICE_ID_KEY];
...
+ (void)setKiwiDeviceId:(NSNumber *)kiwiDeviceId {
    NSNumber *currentKiwiDeviceId = [DeviceUtils getKiwiDeviceIdAsNumber];
    if (kiwiDeviceId != nil && [currentKiwiDeviceId longLongValue] != [kiwiDeviceId longLongValue]) {
        KIWI_DEVICE_ID_VAL = [kiwiDeviceId copy];
        [[UserDefaultsUtil sharedInstance] setObject:KIWI_DEVICE_ID_VAL forKey:KIWI_DEVICE_ID_KEY];
